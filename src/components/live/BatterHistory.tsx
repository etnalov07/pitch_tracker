  ...
  const loadHistory = async () => {
    try {
      ...
      const data = await analyticsService.getBatterHistory(
        batterId,
        showAllTime ? undefined : pitcherId,
        limit
      );
      setHistory(data);
    } catch (error) {
              ...
              <AtBatCard key={ab.id}>
                ...
                <AtBatDetails>
                  <Detail>
                    ...
                    <DetailValue>{ab.balls}-{ab.strikes}</DetailValue>
                  </Detail>
                  <Detail>
...
const AtBatResult = styled.span<{ result?: string }>`
  ...
  background-color: ${props => {
    ...
    if (props.result.includes('single') || props.result.includes('double') ||
        props.result.includes('triple') || props.result.includes('home_run')) {
...
const PitchBadge = styled.span<{ result: string }>`
  ...
  background-color: ${props => {
    switch (props.result) {
      case 'ball': return theme.colors.gray[300];
      case 'called_strike': return theme.colors.green[200];
      case 'swinging_strike': return theme.colors.red[200];
      case 'foul': return theme.colors.yellow[200];
      case 'in_play': return theme.colors.primary[200];
      default: return theme.colors.gray[200];
    }
  }};
  color: ${props => {
    switch (props.result) {
      case 'ball': return theme.colors.gray[800];
      case 'called_strike': return theme.colors.green[800];
      case 'swinging_strike': return theme.colors.red[800];
      case 'foul': return theme.colors.yellow[800];
      case 'in_play': return theme.colors.primary[800];
