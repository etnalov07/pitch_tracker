...
import { Invite, JoinRequest, Team } from '../../types';
...
export const fetchTeamInvites = createAsyncThunk('invites/fetchByTeam', async (teamId: string, { rejectWithValue }) => {
    try {
...
export const fetchInviteByToken = createAsyncThunk('invites/fetchByToken', async (token: string, { rejectWithValue }) => {
    try {
...
export const acceptInvite = createAsyncThunk('invites/accept', async (token: string, { rejectWithValue }) => {
    try {
...
export const revokeInvite = createAsyncThunk('invites/revoke', async (inviteId: string, { rejectWithValue }) => {
    try {
        await invitesApi.revokeInvite(inviteId);
        return inviteId;
    } catch (error: unknown) {
        return rejectWithValue(getErrorMessage(error, 'Failed to revoke invite'));
    }
});

export const searchTeams = createAsyncThunk('invites/searchTeams', async (query: string, { rejectWithValue }) => {
    try {
...
export const fetchMyJoinRequests = createAsyncThunk('invites/fetchMyJoinRequests', async (_, { rejectWithValue }) => {
    try {
...
export const denyJoinRequest = createAsyncThunk('invites/denyJoinRequest', async (requestId: string, { rejectWithValue }) => {
    try {
    ...
    extraReducers: (builder) => {
        builder
            .addCase(createInvite.fulfilled, (state, action) => {
                state.teamInvites.unshift(action.payload);
            })
        ...
        builder.addCase(revokeInvite.fulfilled, (state, action) => {
            const idx = state.teamInvites.findIndex((i) => i.id === action.payload);
            if (idx !== -1) state.teamInvites[idx].status = 'revoked';
        });

        builder.addCase(searchTeams.fulfilled, (state, action) => {
            state.searchResults = action.payload;
        });

        builder.addCase(createJoinRequest.fulfilled, (state, action) => {
            state.myJoinRequests.unshift(action.payload);
        });

        builder.addCase(fetchMyJoinRequests.fulfilled, (state, action) => {
            state.myJoinRequests = action.payload;
        });

        builder.addCase(fetchTeamJoinRequests.fulfilled, (state, action) => {
            state.teamJoinRequests = action.payload;
        });

        builder.addCase(approveJoinRequest.fulfilled, (state, action) => {
            state.teamJoinRequests = state.teamJoinRequests.filter((r) => r.id !== action.payload);
        });

        builder.addCase(denyJoinRequest.fulfilled, (state, action) => {
            state.teamJoinRequests = state.teamJoinRequests.filter((r) => r.id !== action.payload);
        });
    },
});
