    ...
    const loadChart = useCallback(
        async (nextView: BatterView) => {
            ...
            try {
                await Promise.all([
                    needLocations
                        ? analyticsApi.getPitchLocations(batter.batter_id, pitcherId).then(setPitchLocations)
                        : Promise.resolve(),
                    ...
                    {view === 'charts' && (
                        <View style={styles.chartContainer}>
                            ...
                            {!chartLoading && (
                                <>
                                    {pitchLocations ? (
                                        <HeatMapView pitches={pitchLocations} bats={batter.bats} />
                                    ) : (
                                        <Text style={styles.chartLoading}>No pitch location data.</Text>
                                    )}
