...
export class BullpenService {
  ...
  async getPitchesBySession(sessionId: string): Promise<BullpenPitch[]> {
    const result = await query(
      'SELECT * FROM bullpen_pitches WHERE session_id = $1 ORDER BY pitch_number ASC',
      [sessionId]
    );
    return result.rows;
  }
  ...
  async getSessionSummary(sessionId: string): Promise<BullpenSessionSummary | null> {
    ...
    const pitchesWithTarget = pitches.filter((p: any) =>
      p.target_x != null && p.target_y != null && p.actual_x != null && p.actual_y != null
    );
    const accuratePitches = pitchesWithTarget.filter((p: any) => {
      const distance = Math.sqrt(
        Math.pow(p.actual_x - p.target_x, 2) + Math.pow(p.actual_y - p.target_y, 2)
      );
      return distance <= TARGET_ACCURACY_THRESHOLD;
    });
    const targetAccuracy = pitchesWithTarget.length > 0
      ? Math.round((accuratePitches.length / pitchesWithTarget.length) * 100)
      : null;
    ...
    const pitchTypeBreakdown = Array.from(typeMap.entries()).map(([pitch_type, stats]) => ({
      ...
      balls: stats.balls,
      avg_velocity: stats.velocities.length > 0
        ? Math.round((stats.velocities.reduce((a, b) => a + b, 0) / stats.velocities.length) * 10) / 10
        : null,
      top_velocity: stats.velocities.length > 0
        ? Math.max(...stats.velocities)
        : null,
    }));
  ...
  async getPitcherBullpenLogs(pitcherId: string, limit = 20, offset = 0): Promise<{
